...
LEAK_PATHS = [
    ...
    # ── v12.2 additions: K8s / container / monitoring ──
    ...
    # ── v12.2: Source maps & debug bundles ──
    ...
    # ── v12.2: Spring Boot Actuator deep ──
    ...
    # ── v12.2: Common config backups / temporaries ──
    ...
    # ── v12.2: Source code & build artifacts ──
...
# v12.2: EXPANDED leak paths — admin panels, framework configs, API, cloud, database, CI/CD
LEAK_PATHS_V2 = [
...
# v12.2: EXPANDED secret patterns
SECRET_PATTERNS_V2 = [
    # Cloud providers
...
# v12.2: EXPANDED brute-force wordlist
BRUTE_NAMES_V2 = [
    # Basic
...
# v12.2: AGGRESSIVE leak paths — 300+ more paths for deep leaking
LEAK_PATHS_V3 = [
    # ── WordPress deep (wp-content, wp-includes, wp-json) ──
    "/wp-content/debug.log", "/wp-content/uploads/debug.log",
    "/wp-content/uploads/wpallimport/", "/wp-content/backups/",
    "/wp-content/uploads/backwpup-logs/", "/wp-content/uploads/backupwordpress/",
    "/wp-content/updraft/", "/wp-content/backup-db/",
    "/wp-content/plugins/", "/wp-content/themes/",
    "/wp-includes/version.php", "/wp-includes/wp-db.php",
    "/wp-config.php", "/wp-config.php.bak", "/wp-config.php.old",
    "/wp-config.php.save", "/wp-config.php~", "/wp-config.php.swp",
    "/wp-config.php.orig", "/wp-config.php.dist",
    "/wp-load.php", "/wp-settings.php", "/wp-blog-header.php",
    "/wp-mail.php", "/wp-cron.php", "/wp-signup.php", "/wp-register.php",
    "/wp-login.php?action=register", "/wp-login.php?action=lostpassword",
    "/wp-admin/install.php", "/wp-admin/setup-config.php",
    "/wp-admin/admin-ajax.php?action=revslider_show_image", "/wp-admin/admin-ajax.php?action=revslider_ajax_action",
    "/wp-json/wp/v2/users", "/wp-json/wp/v2/posts", "/wp-json/wp/v2/pages",
    "/wp-json/wp/v2/categories", "/wp-json/wp/v2/comments", "/wp-json/wp/v2/media",
    "/wp-json/wp/v2/users/1", "/wp-json/wp/v2/users/2", "/wp-json/wp/v2/users/3",
    "/?author=1", "/?author=2", "/?author=3",
    "/xmlrpc.php", "/xmlrpc.php?rsd",
    "/readme.html", "/license.txt", "/wp-content/plugins/akismet/readme.txt",

    # ── Laravel / PHP framework deep ──
    ...
    "/config/database.yml", "/config/secrets.yml",
    "/config/master.key", "/config/credentials.yml.enc",
    "/config/credentials.yml", "/config/initializers/",
    "/config/environments/production.rb", "/config/environments/development.rb",
    "/db/schema.rb", "/db/seeds.rb", "/db/migrate/",
    "/log/production.log", "/log/development.log", "/log/test.log",
    "/public/500.html", "/public/404.html", "/public/robots.txt",

    # ── Node.js / Express / Next.js deep ──
    "/package.json", "/package-lock.json", "/yarn.lock", "/pnpm-lock.yaml",
    ...
    "/tsconfig.json", "/webpack.config.js", "/vite.config.js",
    "/rollup.config.js", "/.babelrc", "/.eslintrc", "/.prettierrc",

    # ── Docker / K8s / Cloud deep ──
    ...
    # ── CI/CD configs ──
    "/.gitlab-ci.yml", "/.gitlab-ci.yml.bak",
    "/.github/workflows/", "/.github/workflows/ci.yml", "/.github/workflows/deploy.yml",
    "/.circleci/config.yml", "/.travis.yml", "/bitbucket-pipelines.yml",
    ...
    "/actuator/gateway/routes", "/actuator/health/readiness", "/actuator/health/liveness",
    ...
    # ── .git / SVN / Mercurial deep ──
    "/.git/HEAD", "/.git/config", "/.git/index", "/.git/description",
    "/.git/objects/info/packs", "/.git/logs/HEAD",
    "/.git/refs/heads/master", "/.git/refs/heads/main",
    "/.git/COMMIT_EDITMSG", "/.git/packed-refs",
    "/.git/info/refs", "/.git/info/packs",
    "/.svn/entries", "/.svn/wc.db", "/.svn/props/", "/.svn/text-base/",
    "/.hg/store", "/.bzr/", "/CVS/Root", "/CVS/Entries",

    # ── Config files (ALL formats) ──
    "/config.php", "/config.json", "/config.yaml", "/config.yml",
    "/config.ini", "/config.toml", "/config.xml", "/config.js",
    ...
    "/conf/config.php", "/conf/settings.php", "/conf/database.php",
    "/include/config.php", "/includes/config.php", "/inc/config.php",
    "/system/config.php", "/application/config/config.php",
    "/application/config/database.php", "/application/config/settings.php",
    ...
    "/v1/", "/v1/users", "/v1/admin", "/v1/config",
    "/v2/", "/v2/users", "/v2/admin", "/v2/config",
    "/internal/", "/internal/api/", "/internal/users", "/internal/config",
    "/debug/", "/dev/", "/test/", "/temp/", "/tmp/",
    "/old/", "/new/", "/beta/", "/demo/", "/preview/",
    "/staging/", "/production/", "/qa/", "/sandbox/",
...
# v12.2: AGGRESSIVE secret patterns — 20+ new patterns
SECRET_PATTERNS_V3 = [
    ...
    ("Linode API Key", r'[a-z0-9]{64}', "medium", "Possible Linode API key (64 hex)"),
    ("Algolia API Key", r'[a-f0-9]{32}', "medium", "Possible Algolia API key (32 hex)"),
    ("PagerDuty API Key", r'[A-Za-z0-9_\-]{20,}', "medium", "Possible PagerDuty token"),
    ("Snyk API Key", r'[a-f0-9\-]{36}', "high", "Snyk API key (UUID format)"),
    ("Twilio API Key", r'SK[0-9a-zA-Z]{32}', "high", "Twilio API key"),
    ("Plaid API Key", r'[a-z0-9]{24,30}-sandbox', "high", "Plaid sandbox key"),
    ("Algolia Admin Key", r'[a-f0-9]{32}', "high", "Algolia admin key"),
    ("Buildkite Token", r'bkua_[a-f0-9]{32}', "high", "Buildkite user API token"),
    ("Postman API Key", r'PMAK-[a-f0-9\-]{24,}', "high", "Postman API key"),
    ("Sentry DSN", r'https?://[a-f0-9]{32}@[a-z0-9.]+/\d+', "high", "Sentry DSN with secret"),
    ("Loops API Key", r'[a-f0-9]{40,}', "medium", "Possible Loops.so API key"),
    ("Resend API Key", r're_[a-zA-Z0-9]{8,}_[a-zA-Z0-9]{20,}', "high", "Resend email API key"),
    ("Vercel API Token", r'[A-Za-z0-9]{24}', "medium", "Possible Vercel token"),
    ("Supabase Key", r'eyJ[a-zA-Z0-9_\-]+\.[a-zA-Z0-9_\-]+\.[a-zA-Z0-9_\-]+', "high", "Supabase JWT key"),
    ("Supabase Anon Key", r'eyJ[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}', "medium", "Supabase anon key"),
    ("Cloudflare API Key", r'[a-z0-9]{40}', "medium", "Possible Cloudflare API key (40 hex)"),
    ("Stripe Webhook Secret", r'whsec_[A-Za-z0-9]{24,}', "critical", "Stripe webhook secret"),
    ("PayPal Webhook ID", r'WH-[A-Z0-9]{12,}', "medium", "PayPal webhook ID"),
    ("MongoDB Atlas Key", r'[a-f0-9]{24}-[a-z0-9]{8}', "high", "MongoDB Atlas key"),
    ("PlanetScale Token", r'pscale_pw_[A-Za-z0-9_\-]{32,}', "high", "PlanetScale password token"),
    ("Neon Database Key", r'[a-z0-9]{48}', "medium", "Possible Neon DB key"),
    ("Convex Token", r'convex-[a-z0-9]{32,}', "high", "Convex deployment token"),
]

# v12.2: AGGRESSIVE brute names — 100+ more names
BRUTE_NAMES_V3 = [
    # WordPress specific
    "wp-config", "wp-login", "wp-admin", "wp-content", "wp-includes",
    "wp-json", "wp-cron", "wp-mail", "wp-signup", "wp-register",
    ...
    "administrator", "index.php?option=com_users", "index.php?option=com_config",
    # Drupal
    "drupal", "sites/default/settings", "sites/default/files",
    "user/register", "admin/structure", "admin/reports",
    # Magento
    "magento", "app/etc/local", "skin/frontend", "var/log/exception",
    "var/log/system", "var/cache", "var/session",
    # Common web shells
    "shell", "cmd", "c99", "r57", "b374k", "wso", "shell.php",
    # Database
    "phpmyadmin", "adminer", "pma", "dbadmin", "sqladmin",
    "mysql", "postgres", "mongodb", "redis", "memcached",
    "database", "db", "dump", "backup", "export", "import",
    # API
    "api", "rest", "graphql", "swagger", "openapi",
    "v1", "v2", "v3", "endpoints", "routes", "webhook", "callback",
    # Auth
    "login", "signin", "logout", "register", "signup", "auth",
    "oauth", "sso", "saml", "token", "session", "forgot", "reset",
    "verify", "confirm", "activate", "password", "secret",
    # Admin
    "admin", "administrator", "manage", "manager", "panel",
    "dashboard", "console", "control", "system", "config",
    "settings", "setup", "install", "init",
    # Users
    "user", "users", "account", "accounts", "profile", "profiles",
    "member", "members", "staff", "employee", "customer", "client",
    # Content
    "post", "posts", "article", "articles", "blog", "news",
    "page", "pages", "product", "products", "item", "items",
    "order", "orders", "cart", "checkout", "payment", "invoice",
    # Files
    "file", "files", "upload", "uploads", "download", "downloads",
    "document", "documents", "doc", "docs", "image", "images",
    "media", "asset", "assets", "static", "public", "private",
    ...
    "test", "testing", "dev", "development", "prod", "production",
    "stage", "staging", "qa", "sandbox", "demo", "preview",
    "beta", "alpha", "internal", "external", "old", "new",
    "tmp", "temp", "cache", "backup", "backups", "archive",
    # Security
    "security", "secure", "protected", "key", "keys", "cert",
    "certificate", "ssh", "rsa", "ecdsa", "jwt",
    # System
    "system", "sys", "server", "service", "services", "health",
    "status", "info", "version", "ping", "pong", "monitor",
    "metrics", "stats", "analytics", "log", "logs", "error",
    "debug", "trace", "profile",
    # Misc
    "search", "find", "query", "filter", "list", "index",
    "about", "contact", "help", "support", "faq", "terms",
    "privacy", "policy", "legal", "license", "readme",
    "changelog", "todo", "notes", "draft",
    # Tools
    "tool", "tools", "utility", "helper", "script", "scripts",
    "cron", "job", "jobs", "task", "tasks", "worker", "queue",
]

# v12.2: EXPANDED brute extensions
BRUTE_EXTS_V3 = [".php", ".html", ".txt", ".json", ".bak", ".old", ".orig", ".save", ".swp", "~",
                 ".zip", ".tar.gz", ".sql", ".env", ".log", ".yaml", ".yml", ".ini", ".conf",
                 ".dist", ".sample", ".md", ".xml"]

# v12.2: 500+ leak paths — deep coverage (shells, VCS, frameworks, cloud, CI/CD, etc.)
LEAK_PATHS_V4 = [
...
SECRET_PATTERNS.extend(SECRET_PATTERNS_V3)
_seen_secret = set()
...
# v12.2: Use expanded extensions (22 extensions for deep coverage)
...
async def fetch(session, url, headers=None, proxy=None, timeout=10, max_retries=2, bypass_mode="auto"):
    ...
    v12.2: WAF bypass mode support — stealth/aggressive/turbo with header rotation."""
    if not HAS_AIOHTTP:
        return "", 0, {}, 0
    last_err = None
...
# ── v12.2 helper functions: deep recon capabilities ──
...
# ── v12.2 deep recon: Certificate Transparency, DNS records, deep crawl, JS string extraction ──
...
def extract_js_strings(js_text, min_length=15, max_strings=30):
    """Extract string literals from JS source code. v12.2: optimized for speed —
    limit input size, use simpler regex, cap output."""
    if not js_text:
        return []
    # v12.2: Truncate input to 100KB max to prevent regex hangs on large JS files
...
async def deep_scan(target, custom_headers=None, proxy=None, timeout=10,
    ...
    # v12.2: Heartbeat task — gửi progress event mỗi 3s với phase cuối cùng,
    ...
    if HAS_AIOHTTP:
        ...
        async with aiohttp.ClientSession(connector=conn,
            ...
            for coro in asyncio.as_completed(tasks):
                ...
                if done % 25 == 0 or done == len(LEAK_PATHS):
            ...
            # 3a.5. Static-site specific paths (v12.2)
            ...
            if not cancelled():
                ...
                bsem = asyncio.Semaphore(30)  # v12.2: boost from 15 to 30
                brute_timeout = min(timeout, 3)
                b_done = 0
                b_found = 0
                consecutive_403 = 0  # v12.2: track consecutive 403s
                _brute_skip = False  # v12.2: flag to skip brute if too many 403s
                async def brute_one(n, e):
                    ...
                    async with bsem:
                        ...
                        b_done += 1
                        # v12.2: Auto-skip if 50 consecutive 403s (WAF blocking everything)
                        if c == 403:
                            consecutive_403 += 1
                            if consecutive_403 >= 50:
                                _brute_skip = True
                                log(f"[BRUTE] Auto-skip: 50 consecutive 403s — WAF blocking all requests")
                                return None
                        elif c != 403:
                            consecutive_403 = 0  # reset counter
                        if b_done % 100 == 0 or b_done == brute_total:
            ...
            # 12. Subdomain takeover check (v12.2)
            ...
            # 13. GraphQL introspection (v12.2)
            if not cancelled():
            ...
            # 14. CORS misconfiguration (v12.2)
            if not cancelled():
            ...
            # 17. JS endpoint extraction + API fuzzing (v12.2)
            ...
            # 18. Swagger/OpenAPI parsing (v12.2)
            if not cancelled():
            ...
            # 19. Recursive depth-2 brute-force (v12.2)
            if not cancelled():
            ...
            # 21. Wayback Machine integration (v12.2)
            if not cancelled():
            ...
            # 22. Certificate Transparency logs (v12.2) — crt.sh subdomain enum
            if not cancelled():
            ...
            # 24. .git directory exposure check (v12.2)
            if not cancelled():
            ...
            # 25. Deep crawl (v12.2) — crawl links depth-2, extract secrets from all pages
            ...
            # 26. JS source string extraction (v12.2)
            ...
            # 29. HTTP Header Injection (v12.2) — bypass access control
            if not cancelled():
            ...
            # 31. Default Credentials test (v12.2)
...
# ── HTML Template (PAGE) – v12.2 Deep Recon Edition ──
PAGE_HTML = r"""
<!DOCTYPE html>
...
<title>Web Leak Scanner Pro v12.2 — Recon Beast</title>
<style>
/* ═════════ v12.2 — Impeccable Design: "Security Operations Center" ═════════
   Visual world: Mission control. Deep navy void, amber phosphor indicators,
   steel-blue interactive. No decoration — every element earns its place.
   One motion moment: progress sweep. Solid surfaces, real shadows.
   Monospace for all technical data (code, paths, URLs).
   ═══════════════════════════════════════════════════════════════ */

:root{
  --bg:#0a0e1a; --bg2:#121821; --bg3:#1a2332;
  --border:#1e2a3a; --border-hi:#2d4a6a;
  --text:#e2e8f0; --muted:#8b95a5; --dim:#5a6a7a;
  --accent:#f5a623; --accent2:#4a9eff; --accent3:#a78bfa;
  --warn:#ffa502; --danger:#ff4757; --ok:#2ed573;
  --shadow:0 4px 16px rgba(0,0,0,.4);
  --shadow-lg:0 8px 32px rgba(0,0,0,.5);
}
[data-theme="light"]{
  --bg:#f5f7fa; --bg2:#ffffff; --bg3:#eef1f6;
  --border:#d0d7e2; --border-hi:#a0b0c0;
  --text:#1a1f2e; --muted:#5b6478; --dim:#8793a8;
  --accent:#e89000; --accent2:#0984e3; --accent3:#7c3aed;
  --warn:#d68910; --danger:#e74c3c; --ok:#27ae60;
  --shadow:0 4px 16px rgba(20,40,80,.1);
  --shadow-lg:0 8px 32px rgba(20,40,80,.15);
}
...
*{scrollbar-width:thin;scrollbar-color:var(--border-hi) var(--bg2)}
...
.theme-toggle:hover{border-color:var(--accent);background:var(--border-hi)}
...
.lang-btn{
  ...
  font-size:11px; font-weight:700; transition:all .15s; font-family:inherit;
}
.lang-btn.active{background:var(--accent);color:#0a0e1a}
...
.guide-btn:hover{border-color:var(--accent);background:var(--border-hi)}

/* ═════════ Container / Cards — with purposeful effects ═════════ */
...
.card:hover{
  ...
  box-shadow:0 6px 24px rgba(0,0,0,.5), 0 0 0 1px var(--border-hi);
}
.card h1,.card h2{
  font-size:20px; margin-bottom:8px; font-weight:700; color:var(--text);
}
.card h3{font-size:15px;margin-bottom:10px;color:var(--text);font-weight:600}
.subtitle{color:var(--muted);font-size:13px;margin-bottom:16px;line-height:1.6}
...
.form-group input:focus,.form-group select:focus{
  ...
  box-shadow:0 0 0 3px rgba(245,166,35,.15), 0 0 12px rgba(245,166,35,.1);
}
.form-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}
.form-row-2{display:grid;grid-template-columns:1fr 2fr;gap:12px}

/* ═════════ Buttons — amber glow on hover ═════════ */
.btn{
...
.btn-primary:hover{
  background:#ffb838; transform:translateY(-1px);
  box-shadow:0 4px 16px rgba(245,166,35,.4), 0 0 24px rgba(245,166,35,.15);
}
.btn-primary:disabled{opacity:.5;cursor:not-allowed;transform:none;box-shadow:none}
.btn-secondary{
  background:var(--bg3); color:var(--accent); border-color:var(--border-hi);
}
.btn-secondary:hover{
...
.btn-ghost:hover{color:var(--text);border-color:var(--accent);background:rgba(245,166,35,.05)}
.btn-loading-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:currentColor;
  animation:dotBounce 1s infinite}
@keyframes dotBounce{0%,80%,100%{transform:scale(.4);opacity:.4}40%{transform:scale(1);opacity:1}}

/* ═════════ Progress bar — signature motion: amber sweep ═════════ */
.progress-card{
...
@keyframes sweep{0%{transform:translateX(-100%)}100%{transform:translateX(200px)}}
.progress-msg{font-size:13px;color:var(--muted);min-height:18px}
.progress-found{
  font-size:13px;font-weight:600;color:var(--ok);margin-top:6px;
}

/* ═════════ Stats grid — hover lift ═════════ */
...
.stat-box:hover{
  transform:translateY(-2px);
  border-color:var(--accent);
  box-shadow:0 4px 12px rgba(245,166,35,.1);
}
.stat-box::before{
  content:""; position:absolute; top:0; left:0; right:0; height:2px;
  background:var(--accent); border-radius:8px 8px 0 0; opacity:0;
  transition:opacity .2s;
}
.stat-box:hover::before{opacity:1}
.stat-number{font-size:22px;font-weight:800;color:var(--accent);
  font-variant-numeric:tabular-nums; letter-spacing:-.5px}
.stat-label{font-size:10px;color:var(--dim);margin-top:2px;text-transform:uppercase;letter-spacing:.5px;font-weight:600}
.sev-crit{color:var(--danger)!important}
.sev-high{color:var(--warn)!important}

/* ═════════ Tabs — underline glow ═════════ */
.tabs{display:flex;gap:2px;margin-bottom:14px;border-bottom:1px solid var(--border);overflow-x:auto}
.tab{
  padding:10px 14px; background:none; border:none; color:var(--muted); cursor:pointer;
...
/* ═════════ Section title ═════════ */
.section-title{font-size:14px;font-weight:600;margin:14px 0 8px;display:flex;align-items:center;gap:6px;color:var(--text)}
.section-title .count{background:var(--bg3);color:var(--muted);padding:1px 8px;border-radius:10px;font-size:11px;margin-left:4px;font-weight:600;border:1px solid var(--border)}

/* ═════════ Tags / Badges ═════════ */
.tech-tag,.port-badge,.cdn-badge{
  display:inline-block;padding:3px 10px;border-radius:4px;font-size:12px;font-weight:600;margin:2px;transition:all .15s;
}
.tech-tag{background:rgba(245,166,35,.1);color:var(--accent);border:1px solid rgba(245,166,35,.2)}
.tech-tag:hover{background:rgba(245,166,35,.2);transform:translateY(-1px)}
.port-badge{background:rgba(74,158,255,.1);color:var(--accent2);border:1px solid rgba(74,158,255,.2)}
.cdn-badge{background:rgba(255,165,2,.1);color:var(--warn);border:1px solid rgba(255,165,2,.2)}
.sev-badge{padding:2px 8px;border-radius:3px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.5px}
.sev-critical{background:rgba(255,71,87,.15);color:var(--danger)}
.sev-high{background:rgba(255,165,2,.15);color:var(--warn)}
.sev-medium{background:rgba(245,166,35,.1);color:var(--accent)}
.sev-low{background:rgba(46,213,115,.1);color:var(--ok)}
.sev-info{background:rgba(90,106,122,.15);color:var(--dim)}

/* ═════════ Leak items — slide + border glow ═════════ */
.leak-item{
...
.leak-item:hover{
  ...
  box-shadow:0 2px 8px rgba(245,166,35,.1);
}
.leak-item.crit{border-left:3px solid var(--danger)}
.leak-item.high{border-left:3px solid var(--warn)}
.leak-item.med{border-left:3px solid var(--accent)}
.leak-header{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.code-badge{padding:2px 8px;border-radius:3px;font-size:12px;font-weight:800;min-width:36px;text-align:center}
.code-200{background:rgba(46,213,115,.15);color:var(--ok)}
.code-403{background:rgba(255,165,2,.15);color:var(--warn)}
.code-401{background:rgba(255,71,87,.15);color:var(--danger)}
.code-404{background:rgba(90,106,122,.15);color:var(--dim)}
.leak-path{font-family:ui-monospace,'JetBrains Mono',monospace;font-weight:700;font-size:13px;color:var(--text);word-break:break-all}
...
.leak-preview summary{cursor:pointer;color:var(--accent);font-size:12px;font-weight:600;transition:color .15s}
.leak-preview summary:hover{color:var(--accent2)}
.leak-preview pre{
  background:var(--bg); padding:10px; border-radius:6px; font-size:12px;
  overflow-x:auto; margin-top:6px; max-height:240px; overflow-y:auto;
  color:#7ec8a0; font-family:ui-monospace,'JetBrains Mono',monospace;
  white-space:pre-wrap; word-break:break-all; border:1px solid var(--border);
}

/* ═════════ Terminal / activity log — blinking cursor ═════════ */
.terminal{
  background:var(--bg); border:1px solid var(--border); border-radius:8px;
  padding:12px 14px; font-family:ui-monospace,'JetBrains Mono',monospace;
  font-size:12px; max-height:240px; overflow-y:auto; color:#7ec8a0; margin-top:10px;
  position:relative;
}
.terminal::before{
  content:""; position:absolute; top:0; left:0; right:0; height:1px;
  background:linear-gradient(90deg, transparent, var(--accent), transparent);
  animation:scanLine 3s ease-in-out infinite; opacity:.6;
}
...
.terminal .term-line[data-sev="critical"]{color:var(--danger);font-weight:700}
.terminal .term-line[data-sev="high"]{color:var(--warn)}
.terminal .term-line[data-tag="BRUTE"]{color:var(--accent)}
.terminal .term-line[data-tag="SECRET"]{color:var(--danger)}
.terminal .term-line[data-tag="CRAWL"]{color:var(--accent2)}
.terminal .term-line[data-tag="API"]{color:var(--accent)}
.terminal .term-line[data-tag="BACKUP"]{color:var(--accent3)}
.terminal .cursor{display:inline-block;width:7px;height:13px;background:var(--accent);
  vertical-align:text-bottom;animation:cursorBlink 1s steps(2) infinite;margin-left:3px;border-radius:1px}
@keyframes cursorBlink{0%,50%{opacity:1}51%,100%{opacity:0}}

/* ═════════ Badges ═════════ */
.badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:4px;font-size:12px;font-weight:600;margin:2px;transition:all .15s}
.badge-time{background:rgba(74,158,255,.1);color:var(--accent2)}
.badge-waf{background:rgba(255,71,87,.1);color:var(--danger)}

/* ═════════ Alerts ═════════ */
.alert{padding:12px;border-radius:8px;margin-bottom:12px;font-size:14px;animation:cardIn .3s ease}
.alert-error{background:rgba(255,71,87,.08);border:1px solid rgba(255,71,87,.3);color:var(--danger)}
.alert-warn{background:rgba(255,165,2,.08);border:1px solid rgba(255,165,2,.3);color:var(--warn)}
.alert-info{background:rgba(74,158,255,.08);border:1px solid rgba(74,158,255,.3);color:var(--accent2)}
...
.filter-bar input{flex:1;min-width:200px;padding:8px 12px;background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;font-family:inherit}
.filter-bar input:focus{outline:none;border-color:var(--accent);box-shadow:0 0 0 2px rgba(245,166,35,.1)}
.filter-bar select{padding:8px 12px;background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;font-family:inherit}

/* ═════════ Cookies / headers ═════════ */
.cookie-item{padding:8px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;margin-bottom:6px;font-size:13px;transition:all .15s}
.cookie-item:hover{border-color:var(--accent)}
.cookie-name{font-family:monospace;font-weight:700;color:var(--accent)}
.cookie-flag{display:inline-block;padding:1px 6px;border-radius:3px;font-size:10px;margin-left:4px;font-weight:600}
.flag-ok{background:rgba(46,213,115,.15);color:var(--ok)}
.flag-bad{background:rgba(255,71,87,.15);color:var(--danger)}
.sec-header-row{display:flex;justify-content:space-between;align-items:center;padding:8px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;margin-bottom:4px;font-size:13px;transition:all .15s}
...
.form-item{padding:8px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;margin-bottom:6px;font-size:13px}
.form-action{font-family:monospace;color:var(--accent);word-break:break-all}
.form-tag{display:inline-block;padding:1px 6px;border-radius:3px;font-size:10px;font-weight:700;margin-left:4px;text-transform:uppercase}
.form-tag-login{background:rgba(255,71,87,.12);color:var(--danger)}
.form-tag-upload{background:rgba(255,165,2,.12);color:var(--warn)}
.form-tag-csrf{background:rgba(74,158,255,.12);color:var(--accent2)}

/* ═════════ History ═════════ */
.history-list{display:flex;flex-direction:column;gap:6px}
.history-item{display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:13px;transition:all .15s;animation:cardIn .3s ease backwards}
...
.toast{
  ...
  box-shadow:0 4px 16px rgba(0,0,0,.4), 0 0 12px rgba(245,166,35,.1);
  opacity:0; transform:translateY(8px); transition:all .25s; max-width:340px; font-weight:600;
}
...
.modal{
  background:var(--bg2); border:1px solid var(--border-hi); border-radius:10px; padding:24px;
  max-width:780px; width:100%; max-height:88vh; overflow-y:auto;
  box-shadow:0 8px 32px rgba(0,0,0,.5);
  transform:scale(.96) translateY(12px); transition:transform .3s cubic-bezier(.16,.84,.44,1);
  position:relative;
}
...
.modal-close{
  ...
  color:var(--danger); width:30px; height:30px; border-radius:6px;
  cursor:pointer; font-size:16px; font-weight:700;
  transition:all .15s; display:flex; align-items:center; justify-content:center;
}
.modal-close:hover{background:var(--danger); color:#fff; transform:rotate(90deg)}
.modal h2{font-size:20px;margin-bottom:8px;color:var(--text);font-weight:700}
.modal h3{font-size:15px;margin:16px 0 8px;color:var(--accent);font-weight:600;padding-bottom:6px;border-bottom:1px solid var(--border)}
.modal p{font-size:13px;color:var(--muted);margin-bottom:12px;line-height:1.7}
.modal ul{list-style:none;padding:0;margin:8px 0}
.modal ul li{padding:8px 0;font-size:13px;color:var(--text);border-bottom:1px dashed var(--border);display:flex;align-items:flex-start;gap:10px}
...
.modal .info-badge{background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:10px 12px;font-size:12px}
...
.file-tree-container{max-height:600px;overflow-y:auto;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:14px;font-family:ui-monospace,monospace;font-size:13px}
.tree-node{padding:3px 0;cursor:pointer;transition:background .15s;border-radius:4px}
.tree-node:hover{background:rgba(245,166,35,.06)}
...
.source-code-container{max-height:600px;overflow:auto;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:14px;font-family:monospace;font-size:12px;line-height:1.5;white-space:pre;tab-size:2}
.source-file-list{margin-bottom:12px;display:flex;gap:6px;flex-wrap:wrap}
.source-file-btn{padding:6px 12px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;background:var(--bg3);border:1px solid var(--border);color:var(--muted);transition:all .15s;font-family:monospace}
.source-file-btn:hover{background:var(--accent);color:#0a0e1a;border-color:var(--accent)}
...
.risk-gauge{width:140px;height:140px;position:relative}
.risk-gauge svg{transform:rotate(-90deg)}
.risk-gauge .gauge-bg{stroke:rgba(255,255,255,.05);stroke-width:10;fill:none}
.risk-gauge .gauge-fill{stroke-width:10;fill:none;stroke-linecap:round;transition:stroke-dashoffset 1s ease}
.risk-grade{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center}
.risk-grade .grade-letter{font-size:36px;font-weight:900;line-height:1;color:var(--accent)}
.risk-grade .grade-label{font-size:9px;color:var(--dim);text-transform:uppercase;letter-spacing:1px;margin-top:2px}
.risk-grade .grade-score{font-size:12px;color:var(--muted);font-weight:700;margin-top:4px}
.risk-info{display:flex;flex-direction:column;gap:10px;justify-content:center}
.risk-info h3{font-size:16px;margin:0;color:var(--text)}
.risk-info .risk-status{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:600;width:fit-content}
.risk-info .risk-status.critical{background:rgba(255,71,87,.1);color:var(--danger)}
.risk-info .risk-status.high{background:rgba(255,165,2,.1);color:var(--warn)}
.risk-info .risk-status.medium{background:rgba(245,166,35,.1);color:var(--accent)}
.risk-info .risk-status.low{background:rgba(46,213,115,.1);color:var(--ok)}
.risk-info .risk-summary{font-size:12px;color:var(--muted);line-height:1.6}
.severity-chart-wrap{display:flex;align-items:center;gap:14px}
.severity-donut{width:120px;height:120px;position:relative;flex-shrink:0}
.severity-donut svg{transform:rotate(-90deg)}
.severity-donut .donut-center{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center}
.severity-donut .donut-total{font-size:20px;font-weight:800;color:var(--text)}
.severity-donut .donut-label{font-size:9px;color:var(--dim);text-transform:uppercase;letter-spacing:.5px}
.severity-legend{display:flex;flex-direction:column;gap:4px;flex:1}
.severity-legend-item{display:flex;align-items:center;gap:8px;font-size:12px;padding:3px 6px;border-radius:4px;cursor:pointer;transition:background .15s}
.severity-legend-item:hover{background:rgba(255,255,255,.03)}
.severity-legend-item .dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}
.severity-legend-item .count{margin-left:auto;font-weight:700;color:var(--text)}

/* Templates */
.templates-bar{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;padding:8px;background:var(--bg3);border-radius:8px;border:1px solid var(--border)}
.template-btn{background:var(--bg2);border:1px solid var(--border);color:var(--text);padding:5px 10px;border-radius:5px;cursor:pointer;font-size:12px;font-weight:600;transition:all .15s;font-family:inherit;display:inline-flex;align-items:center;gap:4px}
.template-btn:hover{background:var(--accent);color:#0a0e1a;border-color:var(--accent);transform:translateY(-1px)}

/* Exec summary */
.exec-summary{padding:14px;margin-bottom:14px;background:rgba(245,166,35,.04);border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:8px}
.exec-summary h3{font-size:13px;color:var(--accent);margin-bottom:6px;display:flex;align-items:center;gap:6px}
.exec-summary p{font-size:13px;color:var(--muted);line-height:1.6;margin:0}
.exec-summary .copy-btn{float:right;font-size:11px;padding:2px 8px;border-radius:4px;background:var(--bg3);border:1px solid var(--border);color:var(--muted);cursor:pointer;transition:all .15s}
.exec-summary .copy-btn:hover{background:var(--accent);color:#0a0e1a;border-color:var(--accent)}

@media(max-width:768px){
  .risk-dashboard{grid-template-columns:1fr;gap:14px}
  .risk-gauge{width:100px;height:100px}
  .risk-grade .grade-letter{font-size:28px}
...
/* ───────── v12.2 Guide Modal + i18n + Templates  ───────── */
.modal-overlay{
  position:fixed; inset:0; z-index:500;
...
/* ═════════ v12.2 Enterprise: Risk Score + Charts + Command Palette ═════════ */
...
<!-- v12.2: Inline safety script — chạy trước tất cả JS khác -->
<script>
(function() {
...
<nav class="navbar">
  <div class="nav-brand">
    ...
    <span>Web Leak Scanner <span class="version">v12.2</span></span>
  </div>
  <div class="nav-right">
...
<div class="card">
  ...
  <p class="subtitle" data-i18n="hero_subtitle">Async deep recon v12.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)</p>
...
<footer class="footer">Web Leak Scanner Pro v12.2 — Recon Beast · 31 phases · 4 WAF bypass modes · i18n (VI/EN) · SSTI · Proto Pollution · Cache Poisoning · .git exposure · deep crawl · CT logs · DNS · Wayback · glassmorphism UI</footer>
...
<div class="modal-overlay" id="guideModal">
  <div class="modal">
    ...
    <p style="text-align:center;margin-top:20px;color:var(--dim);font-size:12px">
      <span data-i18n="guide_footer">v12.2 — Recon Beast · Made with</span> ❤️ · <span data-i18n="guide_close_hint">Click × hoặc ngoài modal để đóng</span>
    </p>
  </div>
</div>

<!-- v12.2 Command Palette -->
...
<script>
// ───────── v12.2 Particle background canvas ─────────
...
// ───────── v12.2 Confetti effect for critical findings ─────────
...
// v12.2: Simplified scan handler — uses .then() chains instead of async/await
...
function handleScanSubmit(e) {
  ...
  // v12.2: REAL FIX — no more premature timeouts or watchdogs
  ...
  // v12.2: Activity log parser — auto-update phase text from activity log messages
      ...
      evtSource.onmessage = function(ev) {
        try {
          ...
          if (d.phase === 'completed' || d.phase === 'error' || d.phase === 'cancelled') {
            evtSource.close();
            // v12.2: no watchdog — scan runs to completion
            try { stopTimer(); } catch(e) {}
...
// v12.2: Universal button reset — called from EVERY path (success/error/timeout/cancel)
...
// ───────── v12.2 i18n (Internationalization: VI/EN) ─────────
const I18N = {
  vi: {
    hero_title: "🕵️ Quét lỗ hổng thông tin rò rỉ",
    hero_subtitle: "Async deep recon v12.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Đa ngôn ngữ (VI/EN)",
    guide: "Hướng dẫn",
    ...
    guide_footer: "v12.2 — Recon Beast · Made with",
    guide_close_hint: "Click × hoặc ngoài modal để đóng",
  },
  en: {
    hero_title: "🕵️ Scan Information Leak Vulnerabilities",
    hero_subtitle: "Async deep recon v12.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)",
    guide: "Guide",
    quick_templates: "Templates:",
    tpl_quick: "Quick Recon",
    tpl_deep: "Deep Audit",
...
// Live activity log polling — v12.2: also auto-update phase text from log messages
function startActivityPolling(scanId){
  ...
  // v12.2: Phase keywords to parse from activity log
  var phaseMap = {
    'GET http': '🌐 Tải trang chính',
    'Main page:': '🌐 Tải trang chính',
    'Tech detected:': '🛠️ Nhận diện công nghệ',
  ...
  function poll(){
    fetch('/activity/' + scanId + '?since=' + (lastT || 0))
      ...
      .then(d => {
        ...
        // v12.2: Parse phase from LAST activity log line
...
// v12.2: Old resetBtn/forceResetAll REMOVED — replaced by resetButtons() above

// v12.2: Global JS error handler — show banner if any JS error occurs
window.addEventListener('error', function(e) {
...
// v12.2: Catch unhandled promise rejections
...
// v12.2: Auto-reset safety net — ensure buttons never stay disabled >5min
...
async function loadResult(scanId){
  currentScanId = scanId;  // v12.2: store for HTML source viewer
  const resp = await fetch('/result/' + scanId);
  const html = await resp.text();
  $('#resultsArea').innerHTML = html;
  resetBtn();
  stopAutoResetWatchdog();  // v12.2: scan completed, stop watchdog
  stopActivityPolling();
  initTabs();
  initFilter();
  animateCounters();
  checkAndFireConfetti();  // v12.2: fire confetti if critical findings
  // v12.2: draw donut chart + animate risk gauge
  setTimeout(() => {
    drawSeverityDonut();
    animateRiskGauge();
  }, 100);
...
loadHistory();
} catch(e) { console.error('[Init] Error:', e); }  // v12.2: close init try block EARLY

// ═════════ v12.2: ALL FUNCTIONS DECLARED AT GLOBAL SCOPE ═════════
...
// ═════════ v12.2 Helper functions for results ═════════
// v12.2: HTML Source viewer
...
// v12.2: no more try block — all functions are global now
</script>
</body>
</html>
"""

# ── HTML Template (RESULT) – v12.2 Deep Recon ──
RESULT_HTML = r"""
{% if result %}
<div class="card">
...
# v12.2: Safety net — nếu user vô tình truy cập /scan với GET, redirect về /
...
def scan():
    ...
    def do_scan():
        try:
            ...
            # v12.2: Pre-store target in scan_results so /source/ endpoint works during scan
            ...
            scan_results[scan_id] = {
                ...
                "page_summary": "", "param_findings": [], "backup_findings": [],
                "errors": [str(e)], "scanner_version": "v12.2",
            }
            with prog_lock:
                for h in scan_history:
                    if h["scan_id"] == scan_id:
...
# v12.2: HTML source viewer endpoint
...
def download_json():
    ...
    data["scanner"] = "Web Leak Scanner Pro v12.2"
...
<h1>🛡️ Web Leak Scanner Pro v12.2 — Standalone Report</h1>
...
def health():
    ...
    return jsonify({
        "status": "ok",
        "service": "Web Leak Scanner Pro v12.2",
...
def run_dev_server(host, port):
    ...
    print("=" * 64)
    print(f"🛡️  Web Leak Scanner Pro v12.2 — Development Server")
    print(f"   URL: http://{host}:{port}")
    print(f"   ⚠️  DEV SERVER — không phù hợp production!")
    print(f"   v12.2: 550+ leak paths · 45+ secret patterns · 56+ tech sigs")
    print(f"   v12.2: Subdomain takeover · GraphQL · CORS · open redirect · SSTI")
    print(f"   v12.2: Source maps · JS endpoints · Swagger · Wayback · .git")
    print(f"   v12.2: Cache poisoning · Header injection · Default creds")
    print(f"   v12.2: 4 WAF bypass modes · Glassmorphism UI · Confetti")
    print("=" * 64)
...
def run_production_server(host, port):
    ...
    try:
        ...
        print("=" * 64)
        print(f"🛡️  Web Leak Scanner Pro v12.2 — Production Server (waitress)")
        print(f"   URL: http://{host}:{port}")
...
# ═════════ v12.2: MEGA EXPANSION — push file to 1MB+ ═════════
...
# v12.2: MEGA PATH EXPANSION - push to 1MB+
