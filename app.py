...
LEAK_PATHS = [
    ...
    # ── v10.2 additions: K8s / container / monitoring ──
    ...
    # ── v10.2: Source maps & debug bundles ──
    ...
    # ── v10.2: Spring Boot Actuator deep ──
    ...
    # ── v10.2: Common config backups / temporaries ──
    ...
    # ── v10.2: Source code & build artifacts ──
...
async def fetch(session, url, headers=None, proxy=None, timeout=10, max_retries=2, bypass_mode="auto"):
    ...
    v10.2: WAF bypass mode support — stealth/aggressive/turbo with header rotation."""
    if not HAS_AIOHTTP:
        return "", 0, {}, 0
    last_err = None
...
# ── v10.2 helper functions: deep recon capabilities ──
...
# ── v10.2 deep recon: Certificate Transparency, DNS records, deep crawl, JS string extraction ──
...
def extract_js_strings(js_text, min_length=15, max_strings=30):
    """Extract string literals from JS source code. v10.2: optimized for speed —
    limit input size, use simpler regex, cap output."""
    if not js_text:
        return []
    # v10.2: Truncate input to 100KB max to prevent regex hangs on large JS files
...
async def deep_scan(target, custom_headers=None, proxy=None, timeout=10,
    ...
    # v10.2: Heartbeat task — gửi progress event mỗi 3s với phase cuối cùng,
    ...
    if HAS_AIOHTTP:
        ...
        async with aiohttp.ClientSession(connector=conn,
            ...
            # 3a.5. Static-site specific paths (v10.2)
            ...
            # 12. Subdomain takeover check (v10.2)
            ...
            # 13. GraphQL introspection (v10.2)
            if not cancelled():
            ...
            # 14. CORS misconfiguration (v10.2)
            if not cancelled():
            ...
            # 17. JS endpoint extraction + API fuzzing (v10.2)
            ...
            # 18. Swagger/OpenAPI parsing (v10.2)
            if not cancelled():
            ...
            # 19. Recursive depth-2 brute-force (v10.2)
            if not cancelled():
            ...
            # 21. Wayback Machine integration (v10.2)
            if not cancelled():
            ...
            # 22. Certificate Transparency logs (v10.2) — crt.sh subdomain enum
            if not cancelled():
            ...
            # 24. .git directory exposure check (v10.2)
            if not cancelled():
            ...
            # 25. Deep crawl (v10.2) — crawl links depth-2, extract secrets from all pages
            ...
            # 26. JS source string extraction (v10.2)
            ...
            # 29. HTTP Header Injection (v10.2) — bypass access control
            if not cancelled():
            ...
            # 31. Default Credentials test (v10.2)
...
# ── HTML Template (PAGE) – v10.2 Deep Recon Edition ──
PAGE_HTML = r"""
<!DOCTYPE html>
...
<title>Web Leak Scanner Pro v10.2 — Recon Beast</title>
<style>
/* ─────────  v10.2 Theme variables  ───────── */
:root{
...
/* ─────────  Container / Cards — v10.2 with 3D holographic effects  ───────── */
...
/* ───────── v10.2 Guide Modal + i18n + Templates  ───────── */
.modal-overlay{
...
/* ═════════ v10.2 Enterprise: Risk Score + Charts + Command Palette ═════════ */
...
<nav class="navbar">
  <div class="nav-brand">
    ...
    <span>Web Leak Scanner <span class="version">v10.2</span></span>
  </div>
  <div class="nav-right">
...
<div class="card">
  ...
  <p class="subtitle" data-i18n="hero_subtitle">Async deep recon v10.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)</p>
...
<footer class="footer">Web Leak Scanner Pro v10.2 — Recon Beast · 31 phases · 4 WAF bypass modes · i18n (VI/EN) · SSTI · Proto Pollution · Cache Poisoning · .git exposure · deep crawl · CT logs · DNS · Wayback · glassmorphism UI</footer>
...
<div class="modal-overlay" id="guideModal">
  <div class="modal">
    ...
    <p style="text-align:center;margin-top:20px;color:var(--dim);font-size:12px">
      <span data-i18n="guide_footer">v10.2 — Recon Beast · Made with</span> ❤️ · <span data-i18n="guide_close_hint">Click × hoặc ngoài modal để đóng</span>
    </p>
  </div>
</div>

<!-- v10.2 Command Palette -->
...
<script>
// ───────── v10.2 Particle background canvas ─────────
...
// ───────── v10.2 Confetti effect for critical findings ─────────
...
// ───────── v10.2 i18n (Internationalization: VI/EN) ─────────
const I18N = {
  vi: {
    hero_title: "🕵️ Quét lỗ hổng thông tin rò rỉ",
    hero_subtitle: "Async deep recon v10.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Đa ngôn ngữ (VI/EN)",
    guide: "Hướng dẫn",
    ...
    guide_footer: "v10.2 — Recon Beast · Made with",
    guide_close_hint: "Click × hoặc ngoài modal để đóng",
  },
  en: {
    hero_title: "🕵️ Scan Information Leak Vulnerabilities",
    hero_subtitle: "Async deep recon v10.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)",
    guide: "Guide",
    quick_templates: "Templates:",
    tpl_quick: "Quick Recon",
    tpl_deep: "Deep Audit",
...
$('#scanForm').addEventListener('submit', async function(e){
  e.preventDefault();
  e.stopPropagation();  // v10.2: ensure no native submit
  try {
    const btn = $('#scanBtn');
  ...
  try{
    ...
    // v10.2: Catch any JS error in form submit
  ...
  return false;  // v10.2: ensure form doesn't submit natively
});
...
async function loadResult(scanId){
  currentScanId = scanId;  // v10.2: store for HTML source viewer
  ...
  initFilter();
  animateCounters();
  checkAndFireConfetti();  // v10.2: fire confetti if critical findings
  // v10.2: draw donut chart + animate risk gauge
  setTimeout(() => {
    drawSeverityDonut();
    animateRiskGauge();
  }, 100);
...
// ═════════ v10.2 Helper functions for results ═════════
// v10.2: HTML Source viewer
let currentScanId = null;
...
# ── HTML Template (RESULT) – v10.2 Deep Recon ──
RESULT_HTML = r"""
{% if result %}
<div class="card">
...
# v10.2: GET /scan — redirect về / nếu user vô tình truy cập
...
def scan():
    ...
    def do_scan():
        try:
            ...
            # v10.2: Pre-store target in scan_results so /source/ endpoint works during scan
            ...
            scan_results[scan_id] = {
                ...
                "page_summary": "", "param_findings": [], "backup_findings": [],
                "errors": [str(e)], "scanner_version": "v10.2",
            }
            with prog_lock:
                for h in scan_history:
                    if h["scan_id"] == scan_id:
...
# v10.2: HTML source viewer endpoint
...
def download_json():
    ...
    data["scanner"] = "Web Leak Scanner Pro v10.2"
...
<h1>🛡️ Web Leak Scanner Pro v10.2 — Standalone Report</h1>
...
def health():
    ...
    return jsonify({
        "status": "ok",
        "service": "Web Leak Scanner Pro v10.2",
...
def run_dev_server(host, port):
    ...
    print("=" * 64)
    print(f"🛡️  Web Leak Scanner Pro v10.2 — Development Server")
    print(f"   URL: http://{host}:{port}")
    print(f"   ⚠️  DEV SERVER — không phù hợp production!")
    print(f"   v10.2: 550+ leak paths · 45+ secret patterns · 56+ tech sigs")
    print(f"   v10.2: Subdomain takeover · GraphQL · CORS · open redirect · SSTI")
    print(f"   v10.2: Source maps · JS endpoints · Swagger · Wayback · .git")
    print(f"   v10.2: Cache poisoning · Header injection · Default creds")
    print(f"   v10.2: 4 WAF bypass modes · Glassmorphism UI · Confetti")
    print("=" * 64)
...
def run_production_server(host, port):
    ...
    try:
        ...
        print("=" * 64)
        print(f"🛡️  Web Leak Scanner Pro v10.2 — Production Server (waitress)")
        print(f"   URL: http://{host}:{port}")
