...
LEAK_PATHS = [
    ...
    # ── v11.2 additions: K8s / container / monitoring ──
    ...
    # ── v11.2: Source maps & debug bundles ──
    ...
    # ── v11.2: Spring Boot Actuator deep ──
    ...
    # ── v11.2: Common config backups / temporaries ──
    ...
    # ── v11.2: Source code & build artifacts ──
...
# v11.2: EXPANDED leak paths — admin panels, framework configs, API, cloud, database, CI/CD
LEAK_PATHS_V2 = [
...
# v11.2: EXPANDED secret patterns
SECRET_PATTERNS_V2 = [
    # Cloud providers
...
# v11.2: EXPANDED brute-force wordlist
BRUTE_NAMES_V2 = [
    # Basic
...
# v11.2: EXPANDED brute extensions
...
# v11.2: Reduced extensions — only most common, skip rare ones to speed up scan
BRUTE_EXTS_EXPANDED = [".php", ".html", ".txt", ".json", ".bak", ".old", ".orig", ".zip", ".sql", ".env", ".log", ".yaml"]
...
async def fetch(session, url, headers=None, proxy=None, timeout=10, max_retries=2, bypass_mode="auto"):
    ...
    v11.2: WAF bypass mode support — stealth/aggressive/turbo with header rotation."""
    if not HAS_AIOHTTP:
        return "", 0, {}, 0
    last_err = None
...
# ── v11.2 helper functions: deep recon capabilities ──
...
# ── v11.2 deep recon: Certificate Transparency, DNS records, deep crawl, JS string extraction ──
...
def extract_js_strings(js_text, min_length=15, max_strings=30):
    """Extract string literals from JS source code. v11.2: optimized for speed —
    limit input size, use simpler regex, cap output."""
    if not js_text:
        return []
    # v11.2: Truncate input to 100KB max to prevent regex hangs on large JS files
...
async def deep_scan(target, custom_headers=None, proxy=None, timeout=10,
    ...
    # v11.2: Heartbeat task — gửi progress event mỗi 3s với phase cuối cùng,
    ...
    if HAS_AIOHTTP:
        ...
        async with aiohttp.ClientSession(connector=conn,
            ...
            # 3a.5. Static-site specific paths (v11.2)
            ...
            if not cancelled():
                ...
                bsem = asyncio.Semaphore(30)  # v11.2: boost from 15 to 30
                brute_timeout = min(timeout, 4)
                b_done = 0
                b_found = 0
                consecutive_403 = 0  # v11.2: track consecutive 403s
                _brute_skip = False  # v11.2: flag to skip brute if too many 403s
                async def brute_one(n, e):
                    ...
                    async with bsem:
                        ...
                        b_done += 1
                        # v11.2: Auto-skip if 50 consecutive 403s (WAF blocking everything)
                        if c == 403:
                            consecutive_403 += 1
                        ...
                        if b_done % 50 == 0 or b_done == brute_total:
            ...
            # 12. Subdomain takeover check (v11.2)
            ...
            # 13. GraphQL introspection (v11.2)
            if not cancelled():
            ...
            # 14. CORS misconfiguration (v11.2)
            if not cancelled():
            ...
            # 17. JS endpoint extraction + API fuzzing (v11.2)
            ...
            # 18. Swagger/OpenAPI parsing (v11.2)
            if not cancelled():
            ...
            # 19. Recursive depth-2 brute-force (v11.2)
            if not cancelled():
            ...
            # 21. Wayback Machine integration (v11.2)
            if not cancelled():
            ...
            # 22. Certificate Transparency logs (v11.2) — crt.sh subdomain enum
            if not cancelled():
            ...
            # 24. .git directory exposure check (v11.2)
            if not cancelled():
            ...
            # 25. Deep crawl (v11.2) — crawl links depth-2, extract secrets from all pages
            ...
            # 26. JS source string extraction (v11.2)
            ...
            # 29. HTTP Header Injection (v11.2) — bypass access control
            if not cancelled():
            ...
            # 31. Default Credentials test (v11.2)
...
# ── HTML Template (PAGE) – v11.2 Deep Recon Edition ──
PAGE_HTML = r"""
<!DOCTYPE html>
...
<title>Web Leak Scanner Pro v11.2 — Recon Beast</title>
<style>
/* ─────────  v11.2 Theme variables  ───────── */
:root{
...
/* ─────────  Container / Cards — v11.2 with 3D holographic effects  ───────── */
...
/* v11.2: Ensure disabled buttons still respond to clicks for re-enable */
...
/* ───────── v11.2 Guide Modal + i18n + Templates  ───────── */
.modal-overlay{
...
/* ═════════ v11.2 Enterprise: Risk Score + Charts + Command Palette ═════════ */
...
<!-- v11.2: Inline safety script — chạy trước tất cả JS khác -->
<script>
(function() {
...
<nav class="navbar">
  <div class="nav-brand">
    ...
    <span>Web Leak Scanner <span class="version">v11.2</span></span>
  </div>
  <div class="nav-right">
...
<div class="card">
  ...
  <p class="subtitle" data-i18n="hero_subtitle">Async deep recon v11.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)</p>
...
<footer class="footer">Web Leak Scanner Pro v11.2 — Recon Beast · 31 phases · 4 WAF bypass modes · i18n (VI/EN) · SSTI · Proto Pollution · Cache Poisoning · .git exposure · deep crawl · CT logs · DNS · Wayback · glassmorphism UI</footer>
...
<div class="modal-overlay" id="guideModal">
  <div class="modal">
    ...
    <p style="text-align:center;margin-top:20px;color:var(--dim);font-size:12px">
      <span data-i18n="guide_footer">v11.2 — Recon Beast · Made with</span> ❤️ · <span data-i18n="guide_close_hint">Click × hoặc ngoài modal để đóng</span>
    </p>
  </div>
</div>

<!-- v11.2 Command Palette -->
...
<script>
// ───────── v11.2 Particle background canvas ─────────
...
// ───────── v11.2 Confetti effect for critical findings ─────────
...
// v11.2: Simplified scan handler — uses .then() chains instead of async/await
...
function handleScanSubmit(e) {
  ...
  // v11.2: Shorter timeout (15s) + auto-cancel watchdog
  var controller = new AbortController();
  var timeoutId = setTimeout(function() { controller.abort(); }, 15000);

  // v11.2: Stuck watchdog — nếu 60s không có phase change, auto-cancel + reset
  var _lastPhase = '';
  ...
  fetch('/scan', {method: 'POST', body: formData, signal: controller.signal})
      ...
      evtSource.onmessage = function(ev) {
        try {
          ...
          if (d.phase === 'completed' || d.phase === 'error' || d.phase === 'cancelled') {
            evtSource.close();
            clearInterval(_stuckWatchdog);  // v11.2: clear stuck watchdog
            try { stopTimer(); } catch(e) {}
            try { stopActivityPolling(); } catch(e) {}
    ...
    .catch(function(err) {
      clearTimeout(timeoutId);
      clearInterval(_stuckWatchdog);  // v11.2: clear watchdog
...
// v11.2: Universal button reset — called from EVERY path (success/error/timeout/cancel)
...
// ───────── v11.2 i18n (Internationalization: VI/EN) ─────────
const I18N = {
  vi: {
    hero_title: "🕵️ Quét lỗ hổng thông tin rò rỉ",
    hero_subtitle: "Async deep recon v11.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Đa ngôn ngữ (VI/EN)",
    guide: "Hướng dẫn",
    ...
    guide_footer: "v11.2 — Recon Beast · Made with",
    guide_close_hint: "Click × hoặc ngoài modal để đóng",
  },
  en: {
    hero_title: "🕵️ Scan Information Leak Vulnerabilities",
    hero_subtitle: "Async deep recon v11.2 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)",
    guide: "Guide",
    quick_templates: "Templates:",
    tpl_quick: "Quick Recon",
    tpl_deep: "Deep Audit",
...
// v11.2: Old resetBtn/forceResetAll REMOVED — replaced by resetButtons() above

// v11.2: Global JS error handler — show banner if any JS error occurs
window.addEventListener('error', function(e) {
...
// v11.2: Catch unhandled promise rejections
...
// v11.2: Auto-reset safety net — ensure buttons never stay disabled >5min
...
async function loadResult(scanId){
  currentScanId = scanId;  // v11.2: store for HTML source viewer
  const resp = await fetch('/result/' + scanId);
  const html = await resp.text();
  $('#resultsArea').innerHTML = html;
  resetBtn();
  stopAutoResetWatchdog();  // v11.2: scan completed, stop watchdog
  stopActivityPolling();
  initTabs();
  initFilter();
  animateCounters();
  checkAndFireConfetti();  // v11.2: fire confetti if critical findings
  // v11.2: draw donut chart + animate risk gauge
  setTimeout(() => {
    drawSeverityDonut();
    animateRiskGauge();
  }, 100);
...
loadHistory();
} catch(e) { console.error('[Init] Error:', e); }  // v11.2: close init try block EARLY

// ═════════ v11.2: ALL FUNCTIONS DECLARED AT GLOBAL SCOPE ═════════
...
// ═════════ v11.2 Helper functions for results ═════════
// v11.2: HTML Source viewer
...
// v11.2: no more try block — all functions are global now
</script>
</body>
</html>
"""

# ── HTML Template (RESULT) – v11.2 Deep Recon ──
RESULT_HTML = r"""
{% if result %}
<div class="card">
...
# v11.2: Safety net — nếu user vô tình truy cập /scan với GET, redirect về /
...
def scan():
    ...
    def do_scan():
        try:
            ...
            # v11.2: Pre-store target in scan_results so /source/ endpoint works during scan
            ...
            scan_results[scan_id] = {
                ...
                "page_summary": "", "param_findings": [], "backup_findings": [],
                "errors": [str(e)], "scanner_version": "v11.2",
            }
            with prog_lock:
                for h in scan_history:
                    if h["scan_id"] == scan_id:
...
# v11.2: HTML source viewer endpoint
...
def download_json():
    ...
    data["scanner"] = "Web Leak Scanner Pro v11.2"
...
<h1>🛡️ Web Leak Scanner Pro v11.2 — Standalone Report</h1>
...
def health():
    ...
    return jsonify({
        "status": "ok",
        "service": "Web Leak Scanner Pro v11.2",
...
def run_dev_server(host, port):
    ...
    print("=" * 64)
    print(f"🛡️  Web Leak Scanner Pro v11.2 — Development Server")
    print(f"   URL: http://{host}:{port}")
    print(f"   ⚠️  DEV SERVER — không phù hợp production!")
    print(f"   v11.2: 550+ leak paths · 45+ secret patterns · 56+ tech sigs")
    print(f"   v11.2: Subdomain takeover · GraphQL · CORS · open redirect · SSTI")
    print(f"   v11.2: Source maps · JS endpoints · Swagger · Wayback · .git")
    print(f"   v11.2: Cache poisoning · Header injection · Default creds")
    print(f"   v11.2: 4 WAF bypass modes · Glassmorphism UI · Confetti")
    print("=" * 64)
...
def run_production_server(host, port):
    ...
    try:
        ...
        print("=" * 64)
        print(f"🛡️  Web Leak Scanner Pro v11.2 — Production Server (waitress)")
        print(f"   URL: http://{host}:{port}")
