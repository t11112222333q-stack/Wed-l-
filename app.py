...
LEAK_PATHS = [
    ...
    # ── v10.1 additions: K8s / container / monitoring ──
    ...
    # ── v10.1: Source maps & debug bundles ──
    ...
    # ── v10.1: Spring Boot Actuator deep ──
    ...
    # ── v10.1: Common config backups / temporaries ──
    ...
    # ── v10.1: Source code & build artifacts ──
...
async def fetch(session, url, headers=None, proxy=None, timeout=10, max_retries=2, bypass_mode="auto"):
    ...
    v10.1: WAF bypass mode support — stealth/aggressive/turbo with header rotation."""
    if not HAS_AIOHTTP:
        return "", 0, {}, 0
    last_err = None
...
# ── v10.1 helper functions: deep recon capabilities ──
...
# ── v10.1 deep recon: Certificate Transparency, DNS records, deep crawl, JS string extraction ──
...
def extract_js_strings(js_text, min_length=15, max_strings=30):
    """Extract string literals from JS source code. v10.1: optimized for speed —
    limit input size, use simpler regex, cap output."""
    if not js_text:
        return []
    # v10.1: Truncate input to 100KB max to prevent regex hangs on large JS files
...
async def deep_scan(target, custom_headers=None, proxy=None, timeout=10,
    ...
    # v10.1: Heartbeat task — gửi progress event mỗi 3s với phase cuối cùng,
    ...
    if HAS_AIOHTTP:
        ...
        async with aiohttp.ClientSession(connector=conn,
            ...
            # 3a.5. Static-site specific paths (v10.1)
            ...
            # 12. Subdomain takeover check (v10.1)
            ...
            # 13. GraphQL introspection (v10.1)
            if not cancelled():
            ...
            # 14. CORS misconfiguration (v10.1)
            if not cancelled():
            ...
            # 17. JS endpoint extraction + API fuzzing (v10.1)
            ...
            # 18. Swagger/OpenAPI parsing (v10.1)
            if not cancelled():
            ...
            # 19. Recursive depth-2 brute-force (v10.1)
            if not cancelled():
            ...
            # 21. Wayback Machine integration (v10.1)
            if not cancelled():
            ...
            # 22. Certificate Transparency logs (v10.1) — crt.sh subdomain enum
            if not cancelled():
            ...
            # 24. .git directory exposure check (v10.1)
            if not cancelled():
            ...
            # 25. Deep crawl (v10.1) — crawl links depth-2, extract secrets from all pages
            ...
            # 26. JS source string extraction (v10.1)
            ...
            # 29. HTTP Header Injection (v10.1) — bypass access control
            if not cancelled():
            ...
            # 31. Default Credentials test (v10.1)
...
# ── HTML Template (PAGE) – v10.1 Deep Recon Edition ──
PAGE_HTML = r"""
<!DOCTYPE html>
...
<title>Web Leak Scanner Pro v10.1 — Recon Beast</title>
<style>
/* ─────────  v10.1 Theme variables  ───────── */
:root{
...
/* ─────────  Container / Cards — v10.1 with 3D holographic effects  ───────── */
...
/* ───────── v10.1 Guide Modal + i18n + Templates  ───────── */
.modal-overlay{
...
/* ═════════ v10.1 Enterprise: Risk Score + Charts + Command Palette ═════════ */
...
<nav class="navbar">
  <div class="nav-brand">
    ...
    <span>Web Leak Scanner <span class="version">v10.1</span></span>
  </div>
  <div class="nav-right">
...
<div class="card">
  ...
  <p class="subtitle" data-i18n="hero_subtitle">Async deep recon v10.1 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)</p>
...
<footer class="footer">Web Leak Scanner Pro v10.1 — Recon Beast · 31 phases · 4 WAF bypass modes · i18n (VI/EN) · SSTI · Proto Pollution · Cache Poisoning · .git exposure · deep crawl · CT logs · DNS · Wayback · glassmorphism UI</footer>
...
<div class="modal-overlay" id="guideModal">
  <div class="modal">
    ...
    <p style="text-align:center;margin-top:20px;color:var(--dim);font-size:12px">
      <span data-i18n="guide_footer">v10.1 — Recon Beast · Made with</span> ❤️ · <span data-i18n="guide_close_hint">Click × hoặc ngoài modal để đóng</span>
    </p>
  </div>
</div>

<!-- v10.1 Command Palette -->
...
<script>
// ───────── v10.1 Particle background canvas ─────────
...
// ───────── v10.1 Confetti effect for critical findings ─────────
...
// ───────── v10.1 i18n (Internationalization: VI/EN) ─────────
const I18N = {
  vi: {
    hero_title: "🕵️ Quét lỗ hổng thông tin rò rỉ",
    hero_subtitle: "Async deep recon v10.1 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Đa ngôn ngữ (VI/EN)",
    guide: "Hướng dẫn",
    ...
    guide_footer: "v10.1 — Recon Beast · Made with",
    guide_close_hint: "Click × hoặc ngoài modal để đóng",
  },
  en: {
    hero_title: "🕵️ Scan Information Leak Vulnerabilities",
    hero_subtitle: "Async deep recon v10.1 — 550+ leak paths · 31 phases · 4 WAF bypass modes · SSTI · Prototype Pollution · Cache Poisoning · Glassmorphism UI · Multilingual (VI/EN)",
    guide: "Guide",
    quick_templates: "Templates:",
    tpl_quick: "Quick Recon",
    tpl_deep: "Deep Audit",
...
async function loadResult(scanId){
  currentScanId = scanId;  // v10.1: store for HTML source viewer
  ...
  initFilter();
  animateCounters();
  checkAndFireConfetti();  // v10.1: fire confetti if critical findings
  // v10.1: draw donut chart + animate risk gauge
  setTimeout(() => {
    drawSeverityDonut();
    animateRiskGauge();
  }, 100);
...
// ═════════ v10.1 Helper functions for results ═════════
// v10.1: HTML Source viewer
let currentScanId = null;
...
# ── HTML Template (RESULT) – v10.1 Deep Recon ──
RESULT_HTML = r"""
{% if result %}
<div class="card">
...
def scan():
    ...
    def do_scan():
        try:
            ...
            # v10.1: Pre-store target in scan_results so /source/ endpoint works during scan
            ...
            scan_results[scan_id] = {
                ...
                "page_summary": "", "param_findings": [], "backup_findings": [],
                "errors": [str(e)], "scanner_version": "v10.1",
            }
            with prog_lock:
                for h in scan_history:
                    if h["scan_id"] == scan_id:
...
# v10.1: HTML source viewer endpoint
...
def download_json():
    ...
    data["scanner"] = "Web Leak Scanner Pro v10.1"
...
<h1>🛡️ Web Leak Scanner Pro v10.1 — Standalone Report</h1>
...
def health():
    ...
    return jsonify({
        "status": "ok",
        "service": "Web Leak Scanner Pro v10.1",
...
def run_dev_server(host, port):
    ...
    print("=" * 64)
    print(f"🛡️  Web Leak Scanner Pro v10.1 — Development Server")
    print(f"   URL: http://{host}:{port}")
    print(f"   ⚠️  DEV SERVER — không phù hợp production!")
    print(f"   v10.1: 550+ leak paths · 45+ secret patterns · 56+ tech sigs")
    print(f"   v10.1: Subdomain takeover · GraphQL · CORS · open redirect · SSTI")
    print(f"   v10.1: Source maps · JS endpoints · Swagger · Wayback · .git")
    print(f"   v10.1: Cache poisoning · Header injection · Default creds")
    print(f"   v10.1: 4 WAF bypass modes · Glassmorphism UI · Confetti")
    print("=" * 64)
...
def run_production_server(host, port):
    ...
    try:
        ...
        print("=" * 64)
        print(f"🛡️  Web Leak Scanner Pro v10.1 — Production Server (waitress)")
        print(f"   URL: http://{host}:{port}")
